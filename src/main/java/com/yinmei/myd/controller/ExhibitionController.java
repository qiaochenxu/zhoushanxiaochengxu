...
public class ExhibitionController extends Controller {
    ...
    public void basic(){
        ...
        Page<Record> paginate = Db.paginate(pageNumber, pageSize, "select id,title,thumb,IFNULL(description,' ') description,FROM_UNIXTIME(inputtime) exhibitionTime", " from v9_jibenchenglie WHERE status ='99' ORDER BY exhibitionTime desc");
    ...
    public void  network(){
        ...
        //每页显示数目，默认每页10条
        int pageSize = getParaToInt(2,10);
        Page<Record> paginate = Db.paginate(pageNumber, pageSize, "select id,title,thumb,IFNULL(description,' ') description,FROM_UNIXTIME(inputtime) exhibitionTime", " from v9_wangshangzhanting WHERE status='99' ORDER BY exhibitionTime desc");
        set("paginate",paginate);
        renderJson();
    }
